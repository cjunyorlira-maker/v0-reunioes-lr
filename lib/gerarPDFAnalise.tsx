    ...
    <Document>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoValue}>{formatarData(dataLigacao)}</Text>
          </View>
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Duração:</Text>
            <Text style={styles.infoValue}>{formatarDuracao(duracaoSegundos)}</Text>
          </View>
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Tipo:</Text>
            <Text style={styles.infoValue}>{formatarTipo(tipoLigacao)}</Text>
          </View>
        </View>
        ...
        <View style={styles.pilaresGrid}>
          <View style={styles.pilarBox}>
            <Text style={styles.pilarLabel}>CRÉDITO (valor imóvel)</Text>
            <Text style={styles.pilarValue}>{pilares.credito || '—'}</Text>
          </View>
          <View style={styles.pilarBox}>
            <Text style={styles.pilarLabel}>PARCELA (mensalidade)</Text>
            <Text style={styles.pilarValue}>{pilares.parcela || '—'}</Text>
          </View>
          <View style={styles.pilarBox}>
            <Text style={styles.pilarLabel}>ENTRADA (recurso)</Text>
            <Text style={styles.pilarValue}>{pilares.entrada || '—'}</Text>
          </View>
          <View style={styles.pilarBox}>
            <Text style={styles.pilarLabel}>MOMENTO (urgência)</Text>
            <Text style={styles.pilarValue}>{pilares.momento || '—'}</Text>
          </View>
        </View>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoValue}>{perfil.nivel_interesse || 'indefinido'}</Text>
        </View>
        <View style={styles.infoRow}>
          <Text style={styles.infoLabel}>Reunião:</Text>
          <Text style={styles.infoValue}>{reuniao.marcou ? `✓ Marcada (${reuniao.tipo || 'tipo indefinido'})` : '✗ Não marcada'}</Text>
        </View>
        <View style={styles.infoRow}>
          <Text style={styles.infoLabel}>Falar/Ouvir:</Text>
          <Text style={styles.infoValue}>{qualificacao.proporcao_falar_ouvir || 'N/A'}</Text>
        </View>
        ...
        <View style={styles.boxAlerta}>
          <Text>{analise?.proximo_passo_sugerido || 'Definir próxima ação'}</Text>
        </View>

        <Text style={styles.pageNumber} render={({ pageNumber, totalPages }) => `${pageNumber} / ${totalPages}`} fixed />
        <Text style={styles.footer} fixed>
          LR Multimarcas © 2026 — Sistema de IA — CallID: {callid}
        </Text>
      </Page>
      ...
      <Page size="A4" style={styles.page}>
        ...
        {analise?.pontos_criticos?.length > 0 && (
          <>
            <Text style={styles.subsectionTitle}>⚠️ Pontos para melhorar:</Text>
          ...
          <>
            ...
            {analise.objecoes_cliente.slice(0, 6).map((obj: any, i: number) => (
              <View key={i} style={styles.objecaoCard}>
                <Text style={styles.objecaoTitle}>🗣️ "{obj.objecao || 'N/A'}"</Text>
                ...
                {obj.resposta_vendedor && (
                  <Text style={styles.paragraph}>
                    <Text style={styles.objecaoLabel}>Resposta do vendedor: </Text>
                    {obj.resposta_vendedor}
                  </Text>
                )}
        ...
        {tipoLigacao === 'facebook_grupos' && analise?.script_proxima_ligacao && (
          <>
            ...
            <View style={styles.boxScript}>
              <Text>{analise.script_proxima_ligacao}</Text>
            </View>
          </>
        )}
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.transcricaoBox}>
          <Text>{transcricao || 'Transcrição não disponível'}</Text>
        </View>
