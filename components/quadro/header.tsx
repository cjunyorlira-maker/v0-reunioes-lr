...
import { ChevronLeft, ChevronRight, Plus, LogOut, Trophy, Mic, Phone, CalendarDays, BarChart2 } from "lucide-react"
import { createClient } from "@/lib/supabase/client"

type PeriodoFiltro = "semana" | "producao"
...
export function Header({ weekLabel, onPrevWeek, onNextWeek, onNewLead, periodoFiltro = "semana", onPeriodoChange }: HeaderProps) {
  ...
  return (
    <header className="mx-4 md:mx-6 mt-4 mb-5 backdrop-blur-md border border-white/10 rounded-2xl overflow-hidden" style={{ background: "rgba(0,0,0,0.15)" }}>
      ...
      <div className="flex items-center gap-2 ml-auto flex-shrink-0">
        ...
        {/* Filtro de Período - Semana / Produção */}
        {onPeriodoChange && (
          <div className="flex items-center bg-white/[0.04] backdrop-blur-md border border-[rgba(212,175,55,0.15)] rounded-xl overflow-hidden">
            <button
              onClick={() => onPeriodoChange("semana")}
              className={`flex items-center gap-1.5 px-3 h-10 text-[11px] font-semibold transition-all duration-300 ${
                periodoFiltro === "semana"
                  ? "bg-gradient-to-r from-[#d4af37] to-[#c9a227] text-[#0a0a0a]"
                  : "text-[#8a8070] hover:text-[#f5f0e8] hover:bg-white/[0.05]"
              }`}
            >
            ...
            <button
              onClick={() => onPeriodoChange("producao")}
              className={`flex items-center gap-1.5 px-3 h-10 text-[11px] font-semibold transition-all duration-300 ${
                periodoFiltro === "producao"
        ...
        {/* Week navigation - apenas visível no modo semana */}
        <div className="flex items-center bg-white/[0.04] backdrop-blur-md border border-[rgba(212,175,55,0.15)] rounded-xl overflow-hidden transition-all duration-300 hover:border-[rgba(212,175,55,0.3)] hover:shadow-[0_0_20px_rgba(212,175,55,0.1)]">
          ...
          <span className="text-[12px] font-bold text-[#f5f0e8] min-w-[160px] text-center px-3 bg-gradient-to-r from-transparent via-white/5 to-transparent">
            {periodoFiltro === "producao" ? "21 Abr – 22 Mai" : (weekLabel || "...")}
          </span>
        ...
        >
          ...
          <span className="hidden sm:inline relative z-10">Ligacoes</span>
        </Link>
