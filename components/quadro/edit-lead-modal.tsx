...
export function EditLeadModal({ open, lead, onClose, onSubmit }: EditLeadModalProps) {
  ...
  useEffect(() => {
    if (open && lead) {
      setFormData({
        nome: lead.nome,
        dataHora: `${lead.data}T${lead.hora || "09:00"}`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!lead || !formData.nome || !formData.dataHora || !formData.responsavel) {
      return
    }
